...
**Primary Objective**: Achieve AT LEAST Bronze Medal performance through systematic modeling approach. Quantified Goal: **RMSE = 0.55**
...
5. Now testing 2 strategy:
- 4.1. Model No.4 with GNN features added and finer tuning, expected a 0.58 RMSE, fast to achieve
- 4.2 Spatio Temporal Transformers from Chen et al. (ref:https://arxiv.org/abs/2206.10902, github repo:https://github.com/chenghuang66/s2tnet). This is long, but the top leader reported a 0.52-0.54 RMSE score range.
6. Post Competition-Paused: Geometric GNN yields a 0.580 RMSE
- Kaggle changed to API submission, which severly affected our codebase. Now we restart our work based on the nn/nn_gnn.py. When we submit our work, we need to ensure that the generation of parquet files models, otherwise the API will fail when the model cant be loaded.
